...
if ! git remote show public; then
  clear
  echo "--------> Error: add the public repository as a remote. Run the following: 'git remote add public git@github.com:tink-ab/tink-core-android.git'"
  exit
else
  :
fi
...
gh pr create --repo tink-ab/tink-sdk-core-android --head "$releaseBranch" -t "$prTitlePrefix PFMF-$jiraTicketNumber - Release $newVersion" -b "Release $newVersion" --base $masterBranch -r tink-ab/android-maintainer
clear
...
gh pr create --repo tink-ab/tink-sdk-core-android --head "$releaseBranch" -t "$prTitlePrefix PFMF-$jiraTicketNumber - Release $newVersion" -b "Release $newVersion" --base $devBranch -r tink-ab/android-maintainer
clear
